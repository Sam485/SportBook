...
class StepCourt extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        Text(isGym ? 'Select Trainer' : 'Select Court',
        ...
        isGym ? _trainerGrid(context, p) : _courtGrid(context, p, total, sport),
      ],
    );
  }

  // ── Court grid (2-col with image) ──────────────────────────────────────────
  Widget _courtGrid(BuildContext ctx, BookingProvider p, int total, String sport) {
    final imgs = DataService.courtImages[sport]
        ?? DataService.courtImages['Football']!;
              ...
              child: Stack(fit: StackFit.expand, children: [
                  ...
                  Positioned(top: 8, right: 8,
                    child: Container(
                      ...
                      decoration: const BoxDecoration(
                          shape: BoxShape.circle, color: AppTheme.kAccent),
                ...
                Positioned(bottom: 8, left: 10,
                  child: Column(
                    ...
                    children: [
                      Text('Court $num', style: TextStyle(
                          color: sel ? AppTheme.kAccent : Colors.white,
                          fontSize: 13, fontWeight: FontWeight.w800,
                          shadows: const [Shadow(color: Colors.black87, blurRadius: 4)])),
                      if (sport.isNotEmpty)
                        Text('${DataService.emojiFor(sport)} $sport',
  ...
  // ── Trainer grid ───────────────────────────────────────────────────────────
  Widget _trainerGrid(BuildContext ctx, BookingProvider p) =>
      GridView.builder(
        shrinkWrap: true,
        physics: const NeverScrollableScrollPhysics(),
        gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                  ...
                  Positioned(bottom: 10, left: 10, right: 10,
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(t.specialty, style: const TextStyle(
